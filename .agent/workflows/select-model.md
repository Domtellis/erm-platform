---
description: Select the most efficient Gemini/Claude model from the 6-model catalog.
---
...
5. **Mandatory Reporting**: You MUST include the Model Attribution Badge (e.g., `[MODELOPS: ModelName]`) at the start of every `TaskSummary` and in all `notify_user` messages.

6. **Ledger Update**: Record the task and the model used in the `model_usage_ledger.md` artifact before starting work.
