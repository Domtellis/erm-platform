...
export class AiController {
  ...
  async handleBreachDetected(@Payload() data: any) {
    ...
    this.logger.log(
      `Extracted payload breach_case_id: ${payload.breach_case_id}`,
    );
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Get the AI assessment suggestion for a specific breach case",
  })
  async getSuggestion(@Param("breach_case_id") breachCaseId: string) {
    this.logger.log(
      `Fetching AI suggestion for breach_case_id: ${breachCaseId}`,
    );
    const suggestion = await this.aiService.getSuggestion(breachCaseId);
    if (!suggestion) {
      this.logger.warn(
        `No AI suggestion found for breach_case_id: ${breachCaseId}`,
      );
    } else {
      this.logger.log(
        `Found AI suggestion for breach_case_id: ${breachCaseId}`,
      );
    }
    return suggestion;
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Record human feedback (Accept/Modify/Reject) on an AI suggestion",
  })
