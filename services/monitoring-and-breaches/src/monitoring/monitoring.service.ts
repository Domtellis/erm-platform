...
export class MonitoringService {
  ...
  async submitBreach(dto: CreateBreachSubmissionDto) {
    this.logger.log(
      `Processing breach submission: ${dto.metric_name} with value ${dto.observed_value}`,
    );
    ...
    const finalSeverity =
      evaluatedSeverity !== "none"
        ? evaluatedSeverity
        : dto.severity?.toLowerCase() || "low";
    const slas = this.calculateSLADeadlines(finalSeverity);

    this.logger.log(
      `Breach ${dto.metric_name} evaluated as severity: ${finalSeverity}`,
    );
   ...
   */
  private async evaluateSeverity(
    metricName: string,
    observedValue: number,
    category: string,
  ): Promise<string> {
    const appetiteUrl =
      process.env.APPETITE_SERVICE_URL || "http://appetite-service:4012";
    try {
      this.logger.log(
        `Querying appetite-service for ${metricName} thresholds...`,
      );
      const response = await fetch(
        `${appetiteUrl}/appetites/current?category=${category}`,
      );

      if (!response.ok) {
        this.logger.warn(
          `Could not fetch thresholds from appetite-service: ${response.statusText}`,
        );
        return "none";
      }
      ...
      const threshold = appetite.thresholds.find(
        (t) => t.metric_name === metricName,
      );
      if (!threshold) {
        this.logger.warn(
          `No threshold mapping found for metric: ${metricName}`,
        );
        return "none";
      }
      ...
      const sortedEntries = Object.entries(mappings)
        .map(([k]) => ({ num: Number(k), key: k }))
        .sort((a, b) => {
          // For 'greater-than', high values are worse (descending)
          // For 'less-than', low values are worse (ascending)
          return threshold.operator === ">" ? b.num - a.num : a.num - b.num;
        });

      for (const entry of sortedEntries) {
        if (threshold.operator === ">") {
          if (observedValue >= entry.num) return mappings[entry.key];
      ...
      this.logger.error(
        `Automatic severity evaluation failed: ${error.message}`,
      );
  ...
  async closeBreach(id: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (breach.status !== "decision_approved") {
        this.logger.warn(
          `Rejected closure attempt for breach ${id}: Status is ${breach.status}, expected decision_approved`,
        );
        throw new ForbiddenException(
          "Governance Guardrail: Human certification (Decision Approved) is required before a case can be closed.",
        );
      }
  ...
  async handleRiskAssessmentCreated(payload: any) {
    ...
    try {
      ...
      this.logger.log(
        `Updated breach ${breach_case_id} status to ${newStatus}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to update breach status for ${breach_case_id}: ${error.message}`,
