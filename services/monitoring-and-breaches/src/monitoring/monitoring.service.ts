...
export class MonitoringService {
    ...
    async handleDecisionApproved(payload: any) {
        ...
        this.logger.log(`Received Decision Approved event for decision ${eventData.decision_id}`);
        const { breach_case_id } = eventData;

        if (!breach_case_id) {
            this.logger.warn(`Missing breach_case_id in payload: ${JSON.stringify(payload)}`);
            return;
        }
        ...
        try {
            ...
            this.logger.log(`Updated breach ${breach_case_id} status to decision_approved`);
        } catch (error) {
            this.logger.error(`Failed to update breach status for ${breach_case_id}: ${error.message}`);
        }
    }
    ...
    async handleRiskAssessmentCreated(payload: any) {
        const eventData = payload.data || payload;
        this.logger.log(`Received Risk Assessment Created event for assessment ${eventData.assessment_id}`);
        const { breach_case_id } = eventData;
