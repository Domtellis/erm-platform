...
export class JiraService {
  ...
  async createIssue(plan: any) {
    if (!this.jiraUrl || !this.jiraUser || !this.jiraToken) {
      this.logger.warn(
        "Jira credentials not configured. Skipping issue creation.",
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `Jira Issue Created: ${response.data.key} (${response.data.self})`,
      );
    } catch (error) {
      this.logger.error(`Failed to create Jira issue: ${error.message}`);
      if (error.response) {
        this.logger.error(
          `Jira API Response: ${JSON.stringify(error.response.data)}`,
        );
      }
    }
  }
