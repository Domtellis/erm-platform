...
export class NotificationController {
  ...
  async handleBreachDetected(@Payload() message: any) {
    ...
    this.logger.log(
      `[TRACE] Inbound Type: ${Buffer.isBuffer(message) ? "Buffer" : typeof message}`,
    );
    this.logger.log(
      `[TRACE] Raw Snippet: ${JSON.stringify(message).substring(0, 50)}...`,
    );
    ...
    try {
      ...
      if (typeof event === "string") {
        try {
          ...
          this.logger.log(
            `[TRACE] Double-stringified payload detected and resolved.`,
          );
        } catch (e) {
      ...
      this.logger.log(
        `[TRACE] Parsed Event Keys: ${Object.keys(finalEvent || {}).join(", ")}`,
      );
  ...
  async handleRemediationPlanCreated(@Payload() message: any) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to parse remediation - plan - created event: ${err.message} `,
      );
    }
  }
}
