    ...
    try {
      ...
      return this.prisma.$transaction(async (tx) => {
        ...
        if (decision.decision_type === "mitigation") {
          this.logger.log(
            `[BRIDGE] Automating remediation plan for mitigation decision ${id}`,
          );
          try {
            ...
            if (assessment) {
              ...
              this.logger.log(
                `[BRIDGE SUCCESS] Created remediation plan ${plan.id} for breach ${decision.breach_case_id}`,
              );
            } else {
              this.logger.warn(
                `[BRIDGE WARN] No risk assessment found for breach ${decision.breach_case_id}, automation skipped.`,
              );
            }
          } catch (remError) {
            this.logger.error(
              `[BRIDGE FAILURE] Failed to automate remediation plan: ${remError.message}`,
            );
          }
        }
